...
function vigenere_encrypt(text::String, key::String)
    ...
    repeated_key = repeat(key, div(length(text), length(key)) + 1)[1:length(text)]
    
    encrypted_text = ""
    for (t_char, k_char) in zip(text, repeated_key)
        if t_char in alphabet
            # Находим индексы символов в алфавите
            t_idx = findfirst(==(t_char), alphabet)
            k_idx = findfirst(==(k_char), alphabet)
            
            # Вычисляем новый символ
            new_idx = mod(t_idx + k_idx - 2, length(alphabet)) + 1
            push!(encrypted_text, alphabet[new_idx])
        else
...
# Указываем метод заранее (1, 2 или 3)
choice = "1"  # Замените на "1", "2" или "3" для выбора метода

if choice == "1"
    println("=== Маршрутное перестановление ===")
